...
func calcReservedClusters(
	...
	// unjoinedBindClusters are "unjoined" clusters in `binding.spec.clusters`
	unjoinedBindClusters := bindClusters.Difference(readyClusters).Difference(unreadyClusters)
	...
	if unreadyBindClusters.Len() > 0 {
		if !features.FeatureGate.Enabled(features.Failover) {
		...
		} else if len(candidateClusters) < unreadyBindClusters.Len()+unjoinedBindClusters.Len() {
			...
			if placement.ReplicaScheduling == nil || placement.ReplicaScheduling.ReplicaSchedulingType == policyv1alpha1.ReplicaSchedulingTypeDuplicated {
				...
				// this is usually 1, which could be more than 1 in some corner cases
				reservedNum := unreadyBindClusters.Len() + unjoinedBindClusters.Len() - len(candidateClusters)
				if reservedNum > unreadyBindClusters.Len() {
					reservedNum = unreadyBindClusters.Len()
				}
...
func (g *genericScheduler) selectClusters(
	...
	// check spread constraints only if ReplicaSchedulingType is NOT divided
