...
// federatedKeyWrapper wraps the federated key by the associated work name
type federatedKeyWrapper struct {
	keys.FederatedKey

	// for fetching the associated work
	workName string
}
...
func generateKey(obj interface{}) (util.QueueKey, error) {
	...
	fedKey, err := keys.FederatedKeyFunc(cluster, obj)
	if err != nil {
		return nil, err
	}

	return federatedKeyWrapper{
		FederatedKey: fedKey,
...
func (c *WorkStatusController) syncWorkStatus(key util.QueueKey) error {
	...
	observedObj, err := helper.GetObjectFromCache(c.RESTMapper, c.InformerManager, fedKey.FederatedKey)
	if err != nil {
		if apierrors.IsNotFound(err) {
...
func (c *WorkStatusController) handleDeleteEvent(key federatedKeyWrapper) error {
	...
	return c.recreateResourceIfNeeded(work, key.FederatedKey)
}
